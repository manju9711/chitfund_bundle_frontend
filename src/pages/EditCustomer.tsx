...
export default function EditCustomer() {
  ...
  const checkPhoneExists = async (phone: string): Promise<boolean> => {
    if (!phone || phone === originalPhone) return false;
    try {
      const customers = await apiService.getCustomers();
      return customers.some(c => c.phone === phone && c.id !== id);
    } catch (err) {
...
if (formData.phone.length !== 10) {
  ...
  return;
    ...
    // Check phone number before submitting (only if changed)
    if (formData.phone && formData.phone !== originalPhone) {
      const exists = await checkPhoneExists(formData.phone);
      if (exists) {
        setPhoneError('This phone number already exists. Please use a different number.');
  ...
  return (
    <div>
      <h1 className="mb-5 text-gray-800 text-2xl md:text-3xl font-bold">Edit Customer</h1>
...
/>
          ...
          <div className="flex flex-col sm:flex-row gap-2.5 mt-5">
            <button type="submit" className="btn btn-primary" disabled={saving || !!phoneError}>
